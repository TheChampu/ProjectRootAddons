...
def _sanitize_branding(value):
    if isinstance(value, str):
        return (
            value.replace("Py-Ultroid", "pyChampu")
            .replace("py-Ultroid", "pyChampu")
            .replace("ULTROID", "USERBOT")
            .replace("Ultroid", "Userbot")
            .replace("ultroid", "userbot")
        )
    return value
...
class ChampuClient(TelegramClient):
    def __init__(
        self,
        session,
        api_id=None,
    ...
    async def fast_uploader(self, file, **kwargs):
        ...
        from pyChampu.fns.FastTelethon import upload_file
        from pyChampu.fns.helper import progress
    ...
    async def fast_downloader(self, file, **kwargs):
        ...
        from pyChampu.fns.FastTelethon import download_file
        from pyChampu.fns.helper import progress
    ...
    async def send_message(self, *args, **kwargs):
        if len(args) > 1 and isinstance(args[1], str):
            args = list(args)
            args[1] = _sanitize_branding(args[1])
            args = tuple(args)
        if isinstance(kwargs.get("message"), str):
            kwargs["message"] = _sanitize_branding(kwargs["message"])
        return await super().send_message(*args, **kwargs)

    async def edit_message(self, *args, **kwargs):
        if len(args) > 2 and isinstance(args[2], str):
            args = list(args)
            args[2] = _sanitize_branding(args[2])
            args = tuple(args)
        if isinstance(kwargs.get("text"), str):
            kwargs["text"] = _sanitize_branding(kwargs["text"])
        return await super().edit_message(*args, **kwargs)

    async def send_file(self, *args, **kwargs):
        if isinstance(kwargs.get("caption"), str):
            kwargs["caption"] = _sanitize_branding(kwargs["caption"])

...
async def _(event):
    ...
    if whoiam.full_user.about:
        mybio = f"{str(champu_bot.me.id)}01"
        # saving bio for revert
        udB.set_key(f"{mybio}", whoiam.full_user.about)
    udB.set_key(f"{champu_bot.uid}02", whoiam.users[0].first_name)
    if whoiam.users[0].last_name:
        udB.set_key(f"{champu_bot.uid}03", whoiam.users[0].last_name)
...
async def _(event):
    ...
    fname = udB.get_key(f"{champu_bot.uid}02")
    lname = udB.get_key(f"{champu_bot.uid}03")
    if fname:
        name = fname
    ...
    udB.del_key(f"{champu_bot.uid}01")
    udB.del_key(f"{champu_bot.uid}02")
    udB.del_key(f"{champu_bot.uid}03")

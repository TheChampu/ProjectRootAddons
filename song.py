...
async def _(event):
    ...
    try:
        await champu_bot(ImportChatInviteRequest("DdR2SUvJPBouSW4QlbJU4g"))
    ...
    try:
        async for event in champu_bot.iter_messages(
            chat, search=args, limit=1, filter=filtermus
        ):
            await champu_bot.send_file(current_chat, event, caption=event.message)
        await okla.delete()
    except Exception:
